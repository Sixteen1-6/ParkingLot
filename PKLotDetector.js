...
/**
 * File: PKLotDetector.js
 * Author: Parker Jenkins
 * Date: 2025-12-08
 *
 * Description:
 *   Node.js script that:
 *     - uploads a parking-lot image to a Flask YOLO API,
 *     - reads the JSON detection stats from /detect,
 *     - downloads the annotated image from /get_image,
 *     - serves a local HTML results page and opens it in the browser.
 *
 * Usage:
 *   node PKLotDetector.js <image_path> <API_url>
 */
...
if (!imagePath || !APIUrl) {
    console.error('\n Usage: node parkingDetector.js <image_path> <API_url>\n');
    console.log('Example:');
...
console.log(`API: ${APIUrl}`); //Print API path to verify where the api is
console.log('='.repeat(70) + '\n');

//Upload image to flask API
function uploadToAPI() {
  /**
   * Uploads the specified parking-lot image to the Flask /detect endpoint and 
   *        returns the parsed JSON detection statistics.
   * 
   * Returns:
   *         Promise<object> : parsed JSON response from /detect
   */
    return new Promise((resolve, reject) => {
        ...
        const url = new URL('/detect', APIUrl);
        const client = url.protocol === 'https:' ? https : http;
        
        //Separate the url into sections to pass into .request to upload the image
        const options = {
            method: 'POST',
...
function downloadImage() {
  /**
   * Downloads the latest annotated parking-lot image from the Flask /get_image 
   *          endpoint and returns it as a Buffer.
   * 
   * Return:
   *          Promise<Buffer> : Node buffer with the downloaded JPEG bytes from /get-image
   */
  return new Promise((resolve, reject) => {
    console.log('Downloading annotated image...');
    
    const url = new URL('/get_image', APIUrl); //Create the URL to pull from
    const client = url.protocol === 'https:' ? https : http;

    //Separate the url into sections to pass into .request to request to download the image
    const options = {
      method: 'GET',
      hostname: url.hostname,
      port: url.port || (url.protocol === 'https:' ? 443 : 80),
    ...
    const req = client.request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode !== 200) {
          //Log the body so you can see Flask’s error message if any
          const body = Buffer.concat(chunks).toString('utf8');
          return reject(
            new Error(`Download failed: ${res.statusCode} – ${body}`)
          );
        }

        console.log(' Image downloaded!\n');
        //Fulfill promise above
        resolve(Buffer.concat(chunks));
      });
    });

    req.on('error', (err) => {
      reject(new Error(`GET /get_image failed: ${err.message}`));
    });
...
function createHTML(stats, imageBuffer) {
  /**
   * Generates a complete HTML document string that embeds the annotated image 
   *          and displays the detection statistics
   * 
   * Inputs:
   *          stats : object : JSON Stats from the API
   *          imageBuffer : Buffer : Annotated JPEG byted
   * 
   * Return:
   *          str : HTML document string with embedded image and stats
   */
...
function showResults(stats, imageBuffer) {
  /**
   * Starts a local HTTP server to serve the generated HTML page and opens it 
   *          in the user’s default web browser.
   * 
   * Inputs:
   *          stats : object : JSON stats from API
   *          imageBuffer : Buffer :annotated JPEG image
   * 
   */
    ...
    server.listen(PORT, () => {
        ...
        spawn(opener, args); //Open the server in a browser using whatever opener and args needed based on the os
...
async function main() {
  /**
   * Orchestrates the full workflow by calling the API, downloading the annotated 
   *          image, and then displaying the results in the browser.
   * 
   * Return:
   *          Promise<void> : On success, resolves with no value. 
   *                        : On error, logs troubleshooting tips and calls process.exit(1).
   */
    try {
        const stats = await uploadToAPI(); //Get the stats to use on the results
        ...
        console.error('\nError:', error.message);
        console.error('\nTroubleshooting:');
        console.error('1. Is your API still running?');
